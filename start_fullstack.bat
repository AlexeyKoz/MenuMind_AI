...
echo [0/7] Checking Redis connection...
...
if %ERRORLEVEL% == 0 (
    ...
    echo ⚠️  Redis is NOT running - attempting to start it...
    ...
    if exist "C:\Program Files\Redis\redis-server.exe" (
        ...
        echo.
        echo ================================= WARNING =================================
        echo  Redis is NOT installed or not found in common locations!
        echo  
        echo  Checked locations:
        echo  - C:\Program Files\Redis\redis-server.exe
        echo  - %USERPROFILE%\scoop\apps\redis\current\redis-server.exe
        echo  - C:\redis\redis-server.exe
        echo  
        echo  To install Redis:
        echo  1. Using Scoop: scoop install redis
        echo  2. Using Chocolatey: choco install redis-64
        echo  3. Manual: Download from https://github.com/tporadowski/redis/releases
        echo  4. WSL: wsl sudo service redis-server start
        echo  
        echo  The application will continue WITHOUT Redis.
        echo  Features will work but without caching/background tasks.
        echo ========================================================================
        echo.
...
echo [1/7] Verifying ASGI configuration...
...
echo [2/6] Starting Django Backend Server with ASGI/WebSocket support (Port 8000)...
...
echo [3/6] Waiting 5 seconds for Django to start...
timeout /t 5 /nobreak > nul

echo [4/6] Starting React Frontend (Port 3000)...
start "React Frontend" cmd /k "cd frontend && set BROWSER=none && npm start"

echo [5/6] Starting Test Page Server (Port 8001)...
...
echo   - Background Tasks:     N/A (graceful degradation)
echo ============================================
echo.

REM Browsers will NOT open automatically - user will navigate manually
REM Uncomment the lines below if you want auto-open:
REM start http://localhost:3000
REM start http://localhost:8001/test_backend.html
...
if %ERRORLEVEL% == 1 (
    ...
    taskkill /F /IM cmd.exe /FI "WINDOWTITLE eq Redis Server*" 2>nul
