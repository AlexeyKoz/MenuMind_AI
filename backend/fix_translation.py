...
with open('translation_fixed.txt', 'w', encoding='utf-8') as f:
    f.write(f"Translation created for: {recipe.name}\n")
    f.write(f"Ingredients: {len(translated_ingredients)}\n")
    f.write(
        f"First ingredient: {translated_ingredients[0]['name'] if translated_ingredients else 'None'}\n")
    f.write(f"Steps: {len(translated_steps)}\n")
    if translated_steps and translated_steps[0].get('text'):
        f.write(f"First step: {translated_steps[0]['text'][:50]}\n")

print("Fixed translation created! See translation_fixed.txt")
